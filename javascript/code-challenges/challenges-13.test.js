...
// Solution code here...
  let result = arr.reduce((acc, currVal) => acc.length < currVal.length ? currVal : acc, '');
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let result1 = arr.map(element => element.replace('-', ''));
  let result2 = result1.map(element => element.replace('(', ''));
  let result3 = result2.map(element => element.replace(')', ''));
  let result4 = result3.map(element => element.replace(' ', ''));
